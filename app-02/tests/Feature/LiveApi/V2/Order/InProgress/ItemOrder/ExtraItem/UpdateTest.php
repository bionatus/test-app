...
class UpdateTest extends TestCase
{
    ...
    private string $routeName = LiveApiV2::LIVE_API_V2_ORDER_IN_PROGRESS_ITEM_ORDER_EXTRA_ITEM_UPDATE;
    ...
    public function it_uses_can_policy()
    {
        $this->assertRouteUsesMiddleware($this->routeName,
            ['can:updateExtraItemsInProgress,' . RouteParameters::ORDER]);
    }
    ...
    public function it_updates_the_quantity_of_requested_item_order_of_supply_type()
    {
        ...
        $order             = Order::factory()->usingSupplier($staff->supplier)->approved()->create();
        $supply            = Supply::factory()->visible()->create();
        $requestedQuantity = 5;
        $expectedQuantity  = 2;

        $itemOrder = ItemOrder::factory()->usingOrder($order)->usingItem($supply->item)->notInitialRequest()->create([
