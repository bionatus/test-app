...
class LegacyCalculateJobExecutionTimeTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $supplier = Supplier::factory()->createQuietly(['timezone' => $timezone = 'America/Los_Angeles']);
        ...
        $calculate = new LegacyCalculateJobExecutionTime($order);
        $delayTime = $calculate->execute();

        $expectedDate = Carbon::createFromFormat('Y-m-d H:i:s', $expected, $timezone);
