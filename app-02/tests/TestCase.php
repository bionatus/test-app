...
abstract class TestCase extends BaseTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        if ('sqlite' === DB::getDriverName()) {
            $this->sqliteFunctions();
        }

        Relation::morphMap(RelationsMorphs::MAP);
    }

    private function sqliteFunctions()
    {
        ...
        $pdo->sqliteCreateFunction('acos', function($value) {
            return acos($value);
        }, 1);
        $pdo->sqliteCreateFunction('cos', function($value) {
            return cos($value);
        }, 1);
        $pdo->sqliteCreateFunction('sin', function($value) {
            return sin($value);
        }, 1);
        $pdo->sqliteCreateFunction('radians', function($value) {
            return deg2rad($value);
        }, 1);
        $pdo->sqliteCreateFunction('if', function($condition, $true, $false) {
            return $condition ? $true : $false;
        }, 3);
        $pdo->sqliteCreateFunction('greatest', function($valueA, $valueB) {
