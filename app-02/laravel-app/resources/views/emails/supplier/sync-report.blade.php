...
/** @var \Illuminate\Support\Collection $deletedIds */
/** @var \Illuminate\Support\Collection $errors */
?>

<h2>Stores sync process</h2>
<h3>Times</h3>
...
<div>Deleted: {{ $deletedIds->count() }}</div>
<div>Failed geocode update: {{ $failedGeocodeIds->count() }}</div>
...
@else
    <ul>
        @foreach($warnings as $id => $warningFieldMessages)
            <li>
                <h4>Airtable id: {{ $id }}</h4>
                @php($messages = Illuminate\Support\Collection::make($warningFieldMessages->getMessages())->flatten())
                <ul>
                    @foreach($messages as $message)
                        <li>
                            <div>{{ $message }}</div>
