...
class UserTaggedNotificationTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $this->assertUseTrait(UserTaggedNotification::class, SendsPushNotification::class, ['via']);
    }

    /** @test */
    public function it_can_be_queued()
    {
        $reflection = new ReflectionClass(UserTaggedNotification::class);
    ...
    public function it_uses_database_connection()
    {
        ...
        $notification = new UserTaggedNotification($comment);
    ...
    {
        return [
            //expected, forumValue, settingValue, enabled
    ...
    public function it_creates_an_internal_notification_if_requirements_are_met($enabled)
    {
        ...
        (new UserTaggedNotification($comment))->via($user);

        $internalNotification = [
            'user_id'       => $user->getKey(),
            'message'       => $comment->user->fullName() . ' tagged you in a comment. Tap here to view!',
            'source_event'  => 'user_tagged',
            'source_type'   => 'comment',
    ...
    public function it_has_a_correct_fcm_message_and_payload()
    {
        ...
        $body            = $comment->user->fullName() . ' tagged you in a comment. Tap here to view!';
