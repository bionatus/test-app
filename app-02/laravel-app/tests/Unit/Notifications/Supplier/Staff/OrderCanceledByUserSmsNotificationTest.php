...
use App\Notifications\Supplier\Staff\OrderCanceledByUserSmsNotification;
use Config;
...
class OrderCanceledByUserSmsNotificationTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $reflection = new ReflectionClass(OrderCanceledByUserSmsNotification::class);
    ...
    {
        $job = new OrderCanceledByUserSmsNotification(new Order(), new Staff(), true);
    ...
    public function it_is_sent_via_twilio_channel_if_requirements_are_met(
        ...
        $notification = new OrderCanceledByUserSmsNotification($order, $staff, $shouldSendSupplierSms);
    ...
    public function it_sets_twilio_message()
    {
        ...
        $expected = "BluonLive - Bid#: $order->bid_number at $supplier->address location has been cancelled by {$user->fullName()} from {$user->companyName()}. Do Not Reply to this text.";

        $notification = new OrderCanceledByUserSmsNotification($order, $staff, true);
