...
namespace Tests\Unit\Http\Requests\Api\V3\Auth\Phone\Register\Verify;
...
class InvokeRequestTest extends RequestTestCase
{
    ...
    /** @test */
    public function its_code_must_be_of_verification_type()
    {
        $invalid = AuthenticationCode::factory()->login()->create(['code' => 123456]);

        $request = $this->formRequest($this->requestClass, [RequestKeys::CODE => $invalid->code])
            ->addRouteParameter(RouteParameters::UNVERIFIED_PHONE, $invalid->phone->fullNumber());
        $request->assertValidationFailed();
        $request->assertValidationErrors([RequestKeys::CODE]);
        $request->assertValidationMessages([
