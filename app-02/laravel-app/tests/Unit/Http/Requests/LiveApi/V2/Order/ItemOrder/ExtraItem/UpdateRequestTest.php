...
namespace Tests\Unit\Http\Requests\LiveApi\V2\Order\ItemOrder\ExtraItem;
...
class UpdateRequestTest extends RequestTestCase
{
    ...
    /** @test */
    public function each_uuid_in_items_is_required()
    {
        $request = $this->formRequest($this->requestClass, [RequestKeys::ITEMS => [[]]],
            ['method' => 'patch', 'route' => $this->route]);
    ...
    /** @test */
    public function each_uuid_in_items_must_be_a_string()
    {
    ...
    /** @test */
    public function each_uuid_in_items_must_exist()
    {
        ...
        $request->assertValidationFailed();
        $request->assertValidationErrors([RequestKeys::ITEMS . '.0.uuid']);
        $request->assertValidationMessages(['Each item in item orders must exist. They must belong to an order and must have been added on the initial request.']);
    }

    /** @test */
    public function each_uuid_in_items_must_belong_to_the_order()
    {
        $supplier  = Supplier::factory()->createQuietly();
    ...
    /** @test */
    public function each_uuid_in_items_must_have_been_added_on_the_initial_request()
    {
        $supplier  = Supplier::factory()->createQuietly();
        $order     = Order::factory()->usingSupplier($supplier)->create();
        $itemOrder = ItemOrder::factory()->usingOrder($order)->notInitialRequest()->create();
    ...
    /** @test */
    public function each_uuid_in_items_must_belong_to_an_item_of_supply_type()
    {
    ...
    /** @test */
    public function each_uuid_in_items_must_belong_to_an_item_added_by_tech()
    {
        $itemOrderPart = ItemOrder::factory()
            ->usingOrder($this->order)
            ->usingItem(Item::factory()->part()->create())
    ...
    /** @test */
    public function each_quantity_in_items_is_required()
    {
        $request = $this->formRequest($this->requestClass, [RequestKeys::ITEMS => [[]]],
            ['method' => 'patch', 'route' => $this->route]);
    ...
    /** @test */
    public function each_quantity_in_items_must_be_a_integer()
    {
    ...
    /** @test */
    public function each_quantity_in_items_must_be_grater_than_or_equal_to_0()
    {
    ...
    /** @test */
    public function each_quantity_in_items_must_be_valid()
    {
