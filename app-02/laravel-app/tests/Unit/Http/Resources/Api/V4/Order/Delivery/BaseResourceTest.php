...
namespace Tests\Unit\Http\Resources\Api\V4\Order\Delivery;
...
class BaseResourceTest extends TestCase
{
    ...
    public function it_has_correct_fields_with_pickup()
    {
        ...
        $resource = new BaseResource($orderDelivery);
    ...
    public function it_has_correct_fields_with_shipment_delivery()
    {
        ...
        $shipmentDeliveryPreference->shouldReceive('getRouteKey')->withNoArgs()->once()->andReturn('overnight');
        ...
        $shipmentDelivery->shouldReceive('getAttribute')
            ->with('shipmentDeliveryPreference')
            ->once()
            ->andReturn($shipmentDeliveryPreference);

        $orderDelivery = Mockery::mock(OrderDelivery::class);
        $orderDelivery->shouldReceive('getAttribute')->with('type')->once()->andReturn($type = 'shipment_delivery');
        $orderDelivery->shouldReceive('getAttribute')
            ->with('requested_date')
            ->once()
            ->andReturn($requestedDate = Carbon::now());
