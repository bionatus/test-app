...
class DetailedResourceTest extends TestCase
{
    ...
    public function it_has_correct_fields_with_null_values()
    {
        ...
        $supplier->shouldReceive('getAttribute')->with('supplierHours')->once()->andReturn(Collection::make());
    ...
    public function it_has_correct_fields_with_data()
    {
        ...
        $supplierHour->shouldReceive('getAttribute')->with('day')->times(6)->andReturn('monday');
        $supplierHour->shouldReceive('getAttribute')->with('from')->twice()->andReturn('09:30 am');
        $supplierHour->shouldReceive('getAttribute')->with('to')->twice()->andReturn('05:30 pm');
        ...
        $response        = $resource->resolve();
        $nextWorkingDays = (App::make(GetNextWorkingDays::class, ['supplier' => $supplier]))->execute();
        $data            = [
            'id'                     => $id,
            'name'                   => $name,
            'address'                => $address,
            ...
            'phone'                  => $contactPhone,
            'distance'               => $distance,
            'open_hours'             => SupplierHourResource::collection($nextWorkingDays),
        ];
        $this->assertEquals($data, $response);
        $schema = $this->jsonSchema(DetailedResource::jsonSchema(), false, false);
        $this->assertValidSchema($schema, json_decode(json_encode($response)));
