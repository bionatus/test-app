...
class UpdateTest extends TestCase
{
    ...
    private string $routeName = RouteNames::API_V3_POST_UPDATE;
     ...
     * @throws Exception
     */
    public function it_can_update_its_video_url()
    {
        $post     = Post::factory()->create();
        $route    = URL::route($this->routeName, $post);
        $message  = $this->faker->text(100);
        ...
        $data = Collection::make($response->json('data'));
        $this->assertSame($data['id'], $post->getRouteKey());
        $this->assertSame($data['video_url'], $videoUrl);
        $this->assertSame($post->comments()->count(), $data->get('total_comments'));
     ...
     * @throws Exception
     */
    public function it_deletes_tags_if_tags_are_not_sent()
    {
        $post         = Post::factory()->funny()->create();
        $series       = Series::factory()->create();
        $morePlainTag = PlainTag::factory()->more()->create();
        ...
        $this->assertEquals(0, $postTagsCount);
        $this->assertDatabaseMissing(Tag::tableName(), $tagSeries->toArray());
        $this->assertDatabaseMissing(Tag::tableName(), $tagMore->toArray());
    }
