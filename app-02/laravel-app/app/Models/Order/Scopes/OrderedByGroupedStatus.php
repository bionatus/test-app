...
class OrderedByGroupedStatus implements Scope
{
    public function apply(Builder $builder, Model $model): void
    {
        ...
        $builder->select("$order.*", "$orderSubstatus.order_id")
            ...
            ->orderByRaw("$substatus.status_id = " . Status::STATUS_COMPLETED . " OR $substatus.status_id =" . Status::STATUS_CANCELED)
            ->orderByRaw("$substatus.status_id = " . Status::STATUS_PENDING . " OR $substatus.status_id = " . Status::STATUS_APPROVED)
            ->orderByRaw("$substatus.status_id = " . Status::STATUS_PENDING_APPROVAL);
    }
}
